...
public class NormalPlaying extends Activity implements View.OnClickListener {

    final static long INTERVAL = 700;
    ...
    ArrayList <QuestionNormal> ranListQuesNormal = new ArrayList<>();
    ...
    public void onClick(View view) {
        ...
        if (index < totalQuestion){
            ...
            if (clickedButton.getText().equals(ranListQuesNormal.get(index).getCorrectAnswer())){
    ...
    private void showQuestion(int index) {
        if (index < 10){
            ...
            if (ranListQuesNormal.get(index).getIsImageQuestion().equals("true")){
                // if is image
                Picasso.with(getBaseContext())
                        .load(ranListQuesNormal.get(index).getQuestion())
                        .into(question_image);
                question_image.setVisibility(View.VISIBLE);
                question_text.setVisibility(View.INVISIBLE);
            }
            else{
                question_text.setText(ranListQuesNormal.get(index).getQuestion());
            ...
            btnA.setText(ranListQuesNormal.get(index).getAnswerA());
            btnB.setText(ranListQuesNormal.get(index).getAnswerB());
            btnC.setText(ranListQuesNormal.get(index).getAnswerC());
            btnD.setText(ranListQuesNormal.get(index).getAnswerD());
    ...
    protected void onPostResume() {
        ...
        totalQuestion = Common.questionNormalList.size();

        for(int i=0;i<totalQuestion;i++)
        {
            ranListQuesNormal.add(Common.questionNormalList.get(i));
        }

        Collections.shuffle(ranListQuesNormal);
